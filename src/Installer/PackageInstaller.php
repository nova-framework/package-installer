...
class PackageInstaller extends LibraryInstaller
{
    ...
    public function checkUsage(Composer $composer)
    {
        ...
        $postAutoloadDump = 'Nova\Composer\Installer\PackageInstaller::postAutoloadDump';
     ...
     *
     * Recreates Nova's plugin path map, based on composer information and available app-packages.
     *
    ...
    public static function determinePlugins($packages, $packagesDir = 'packages', $vendorDir = 'vendor')
    {
        ...
        foreach ($packages as $package) {
            if ($package->getType() !== 'nova-package') {
                continue;
            }
        ...
        if (is_dir($pluginsDir)) {
            ...
            foreach ($iterator as $info) {
                ...
                if (is_array($config) && ($config['type'] === 'nova-package')) {
...
PHP;
    ...
    {
        return $vendorDir . DIRECTORY_SEPARATOR . 'nova-packages.php';
    }
        ...
        if (is_null($namespace)) {
            throw new RuntimeException(
                sprintf(
                    ...
                    " as stated in README on https://github.com/nova-framework/packages-installer",
    ...
    {
        return ('nova-package' === $packageType);
    }
    ...
    public function updateConfig($name, $path)
    {
        ...
        if (! isset($config)) {
            $this->io->write(
                'ERROR - `vendor/nova-packages.php` file is invalid. Packages path configuration not updated.'
            );
    ...
    /**
     * Ensure that the vendor/nova-packages.php file exists.
     *
    ...
    protected function ensureConfigFile($path)
    {
        if (file_exists($path)) {
            if ($this->io->isVerbose()) {
                $this->io->write('vendor/nova-packages.php exists.');
            }
...
PHP;
        ...
        if ($this->io->isVerbose()) {
            $this->io->write('Created vendor/nova-packages.php');
        }
    }
